...
PROJECT_NAME = "ZookeeperMergeTest"

CAPITALIZED_PROJECT_NAME = PROJECT_NAME.upper()

# Remote name which points to the GitHub site
PR_REMOTE_NAME = os.environ.get("PR_REMOTE_NAME", "apache-github")
# Remote name which points to szucsvillo git
PUSH_REMOTE_NAME = os.environ.get("PUSH_REMOTE_NAME", "szucsvillo")
# ASF JIRA username
...
GITHUB_OAUTH_KEY = "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"
...
def merge_pr(pr_num, title, pr_repo_desc):
    ...
    #Check the author and the close line
...
def check_jira_env():
    ...
    if JIRA_IMPORTED:
        if JIRA_ACCESS_TOKEN is None:
            continue_maybe("***********************************JIRA_ACCESS_TOKEN is None*****************************")
            ...
            if JIRA_USERNAME.strip() == "" or JIRA_PASSWORD.strip() == "":
                msg ="Neither JIRA_ACCESS_TOKEN nor JIRA_USERNAME and/or JIRA_PASSWORD are set."
                continue_maybe(msg)
    else:
...
def main():
    ...
    if JIRA_IMPORTED:
        if (JIRA_ACCESS_TOKEN is not None) or (JIRA_USERNAME and JIRA_PASSWORD):
            ...
            print("Neither JIRA_ACCESS_TOKEN nor JIRA_USERNAME and JIRA_PASSWORD are set.")
